...
/** @description Database seed script - run with: npx tsx prisma/seed.ts */
...
async function main() {
        ...
        if (!existing) {
            await prisma.productCategory.create({
                data: {
                    ...
                    translations: {
                        create: [
                            { locale: 'en', name: cat.nameEn, description: cat.descEn },
                            { locale: 'ar', name: cat.nameAr, description: cat.descEn },
                        ]
                    }
                }
            });
    ...
    for (const ind of industryCategories) {
        const existing = await prisma.industryCategory.findUnique({ where: { slug: ind.slug } });
        if (!existing) {
            await prisma.industryCategory.create({
                data: {
                    ...
                    translations: {
                        create: [
                            { locale: 'en', name: ind.nameEn },
                            { locale: 'ar', name: ind.nameAr },
                        ]
                    }
                }
            });
